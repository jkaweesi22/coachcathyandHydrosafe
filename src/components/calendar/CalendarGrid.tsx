...
function createDefaultItems(): TodoItem[] {
  return DEFAULT_ITEMS.map((text, i) => ({
    id: `default-${i}`,
    text,
    done: false,
  }));
...
export function CalendarGrid() {
  ...
  useEffect(() => {
    const stored = getFromStorage<CalendarData>(STORAGE_KEYS.CALENDAR);
    setData(stored ?? {});
  }, []);

  useEffect(() => {
    setToStorage(STORAGE_KEYS.CALENDAR, data);
  }, [data]);
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-soft">
        ...
        <div className="mt-2 grid grid-cols-7 gap-1">
          ...
          {Array.from({ length: daysInMonth }, (_, i) => {
            ...
            const dayTotal = dayItems.length || DEFAULT_ITEMS.length;
